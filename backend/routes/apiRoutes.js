                        ...
                        //if (acceptedJobs === 0 ) {
                          ...
                          application
                            ...
                            .then(() => {
                              ...
                              console.log("Application type",application.jobId.jobType)
                            })
                            .catch((err) => {
                              res.status(400).json(err);
                            });
...
router.put("/rating", jwtAuth, async (req, res) => {
  try {
    ...
    // Check if the user is a recruiter or an applicant
    if (user.type === "recruiter") {
      // Check if the recruiter has already rated the applicant for the job
      ...
      if (existingRating) {
        return res.status(400).json({
          message: "You have already rated this applicant for the job",
        });
      }

      // Proceed to add a new rating
      ...
      // Calculate and update the average rating for the applicant
      ...
      if (avgRatingResult.length === 0 || !avgRatingResult[0].average) {
        return res.status(400).json({
      ...
      // Check if the applicant has already rated the job
      const existingRating = await Rating.findOne({
        senderId: user._id,
      ...
      // Proceed to add a new rating
      const acceptedJobCount = await Application.countDocuments({
        userId: user._id,
        jobId: data.jobId,
        status: { $in: ["accepted", "finished"] },
      });
      ...
      // Calculate and update the average rating for the job
      const avgRatingResult = await Rating.aggregate([
        {
          $match: {
...
router.post('/send-message',asyncHandler(async(req, res)=>{
     try{ const {mobile,message} = req.body;
      if(!mobile){
         throw new ApiError(402,"Mobile number and Message is Required");
      }
      const sendSMS = async ()=>{
        const client = new twilio(process.env.TWILIO_SID, process.env.TWILIO_AUTH_TOKEN);
    const twilioMessage = await client.messages.create({ body: message, from:process.env.TWILIO_PHONE_NO, to: process.env.CONTACT });
    console.log("Message Sent", twilioMessage);
